...
class Parser {
	...
	static Graphl TGF(BufferedReader file, Graphl G, boolean directed) throws IOException {
		...
		// if file is blank
		if ((line = file.readLine()) == null) throw new RuntimeException("Unable to read vertices1");
		// get rid of all the opening #'s
		while(line.length() == 0 || line.charAt(0) == '#'){
			if ((line = file.readLine()) == null) throw new RuntimeException("Unable to read vertices2");
		}
		...
		while (line.charAt(0) != '#'){
			...
			if ((line = file.readLine()) == null) throw new RuntimeException("Unable to read vertices3");
		}
